...
This query can be used to determine all the differences between the 'constructed spi' and the spi obtained directly from the `auprparc.ttl_nme` field. Comment out the various conditions at the end to obtain the different types of discrepancies.

```
select
    propnum as ass_num,
    crefno as pcl_num,
    internal_spi as ttl_nme,
    spi as spi_from_parcel_attributes,
    memo
from
(
...
where
spi <> internal_spi and
internal_spi not like '% %' and
(internal_spi like '%\%' or internal_spi like '%CP%' or internal_spi like '%PC%') and
spi not in ( select spi from pc_vicmap_parcel ) and
internal_spi in ( select spi from pc_vicmap_parcel ) and
not ( spi like '%PP5%' or internal_spi like '%PP5%' ) and
not ( spi not like '%~%' and internal_spi like '%~%' ) and
not ( internal_spi like '%PC%' ) and
not ( internal_spi like '%SP%' )
```

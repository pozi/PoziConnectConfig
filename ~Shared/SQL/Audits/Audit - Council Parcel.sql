select
    ...
    case
        ...
        when parish_code = '' and township_code = '' and plan_number = '' and allotment <> '' then 'Invalid: parish and/or township missing for crown description'
        when parish_code <> '' and ( cast ( parish_code as integer ) < 2000 or cast ( parish_code as integer ) > 4005 ) then 'Invalid: parish number not in valid range (' || parish_code || ')'

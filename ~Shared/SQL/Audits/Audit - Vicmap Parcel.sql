select
    ...
    multi_assessment,
    ifnull ( ( select count(*) from pc_council_parcel x where x.spi = vp.spi and length ( vp.spi ) >= 5 ) , '' ) as spi_in_council,
    ...
    ifnull ( ( select count(*) from pc_vicmap_parcel vpx where vpx.spi = vp.spi and length ( vp.spi ) >= 5 ) , 0 ) as spi_in_vicmap,
    ...
    ifnull ( ( select count(*) from pc_council_parcel cp where cp.simple_spi = vp.simple_spi and cp.spi <> vp.spi and length ( vp.spi ) >= 5 ) , 0 ) as partial_spi_in_council,
    ifnull ( ( select count(*) from pc_council_parcel cp where cp.spi = vp.further_description and length ( cp.spi ) >= 5 ) , 0 ) as alt_spi_in_council,

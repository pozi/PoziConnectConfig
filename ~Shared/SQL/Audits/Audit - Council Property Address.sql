select
    ...
    case
        ...
        when length ( building_name ) > 45 then 'Invalid: building_name (' || building_name || ') exceeds limit of 45 characters'
        ...
        when hsa_unit_id <> '' and hsa_flag <> 'Y' then 'Invalid: hsa_flag is required when hsa_unit_id is populated'
        ...
        when floor_type <> '' and hsa_flag = 'Y' then 'Invalid: floor_type value not valid for hotel-style addressing'
        ...
        when floor_prefix_1 <> '' and hsa_flag = 'Y' then 'Invalid: floor_prefix_1 value not valid for hotel-style addressing'
        ...
        when floor_no_1 <> '' and hsa_flag = 'Y' then 'Invalid: floor_no_1 value not valid for hotel-style addressing'
        ...
        when floor_suffix_1 <> '' and hsa_flag = 'Y' then 'Invalid: floor_suffix_1 value not valid for hotel-style addressing'
        ...
        when floor_prefix_2 <> '' and hsa_flag = 'Y' then 'Invalid: floor_prefix_2 value not valid for hotel-style addressing'
        ...
        when floor_no_2 <> '' and hsa_flag = 'Y' then 'Invalid: floor_no_2 value not valid for hotel-style addressing'
        ...
        when floor_suffix_2 <> '' and hsa_flag = 'Y' then 'Invalid: floor_suffix_2 value not valid for hotel-style addressing'
        ...
        when house_suffix_1 <> '' and house_number_1 = '' then 'Invalid: house_number_1 must be present for house_suffix_1 (' || house_suffix_1 || ')'
        ...
        when house_suffix_2 <> '' and house_number_2 = '' then 'Invalid: house_number_2 must be present for house_suffix_2 (' || house_suffix_2 || ')'
    ...
    case
        ...
        when replace ( replace ( num_road_address , '-' , ' ' ) , '''' , '' ) = ( select replace ( num_road_address , '-' , ' ' ) from pc_vicmap_property_address vpa where vpa.propnum = cpa.propnum and vpa.is_primary <> 'N' limit 1 ) then 'Y'
        else 'N'
    ...
    case
        ...
        when ( select vpa.locality_name from pc_vicmap_property_address vpa where vpa.propnum = cpa.propnum and vpa.is_primary <> 'N' limit 1 ) like locality_name || ' (%)' then 'Y'
        else 'N'
    end as locality_match_in_vicmap,
    case
        when replace ( replace ( road_locality , '-' , ' ' ) , '''' , '' ) in ( select replace ( road_locality , '-' , ' ' ) from pc_vicmap_property_address vpa where vpa.propnum = cpa.propnum and vpa.is_primary <> 'N' limit 1 ) then 'Y'
        else 'N'
    ...
    cpa.*,
    ( select vpa.geometry from pc_vicmap_property_address vpa where vpa.propnum = cpa.propnum and vpa.propnum <> 'NCPR' limit 1 ) as geometry

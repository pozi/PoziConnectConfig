select distinct
    ...
    '=hyperlink("https://vicmap.pozi.com/?propertypfi=' || property_pfi || '","https://vicmap.pozi.com/?propertypfi=' || property_pfi || '")' as map_link
    ...
    (
        (
            ( vp.spi not in ( select spi from pc_vicmap_parcel vpx where vpx.propnum in ( select propnum from pc_council_parcel cpx where cpx.spi = vp.spi ) ) or
                (
                    ( select num_parcels_in_prop from pc_vicmap_parcel_property_parcel_count vpppc where vpppc.spi = vp.spi ) > 1 and
                    spi not in ( select spi from pc_council_parcel cp where cp.propnum = vp.propnum )
                )
            ) and
...
order by property_view_pfi
)
where property_pfi not in ( select property_pfi from m1_r_edits )
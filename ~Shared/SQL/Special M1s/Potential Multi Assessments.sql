...
select
	...
	'=hyperlink("https://vicmap.pozi.com/?parcelspi=' || cp.spi || '","https://vicmap.pozi.com/?parcelspi=' || cp.spi || '")' as map_link
from
...
where
    ...
    cp.spi <> '' and
    not ( select vpppc.num_parcels_in_prop from pc_vicmap_parcel_property_parcel_count vpppc where vpppc.spi = cp.spi ) > 1 and
...
where propnum not in ( select propnum from m1 where edit_code = 'A' )

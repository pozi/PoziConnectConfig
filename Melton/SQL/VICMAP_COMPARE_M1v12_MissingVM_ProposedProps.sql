...
CASE
   ...
   CounAddr.fl_suff_1 AS "floor_suffix_1",
	...
	CASE
		...
		ELSE 'A'
	END AS "edit_code",
	'New(Proposed) Council property no to be updated in Vicmap' AS Comments

FROM
    TMP_VM_COMPARE_MissingVMProposedProps_usingSPI AS MissingVMProps 
LEFT JOIN
Temp_PIQA_Address CounAddr ON MissingVMProps.Crefno = CounAddr.crefno

SELECT
	...
	CASE 
      WHEN  DiffVMProps.MultiAssCount =1 THEN ''
      ELSE (Select PR_PFI from TMP_VM_PROPERTY_PARCEL  where TMP_VM_PROPERTY_PARCEL.parcel_spi = DiffVMProps.SPI)
    END AS "property_PFI",
   ...
   '' AS fl_prefix_2,
	...
	CASE
		...
		ELSE 'A'
	END AS "edit_code",
	'Difference in Vicmap props' AS Comments

FROM
    TMP_VM_COMPARE_DiffVMProps_usingSPI AS DiffVMProps 
LEFT JOIN
	
Temp_PIQA_Address CounAddr ON DiffVMProps.Crefno = CounAddr.crefno	
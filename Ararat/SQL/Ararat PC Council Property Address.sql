...
select distinct
    ...
    '' as floor_no_2,
    '' as floor_suffix_2,
    ifnull ( upper ( propertyName ) , '' ) as building_name,
    '' as complex_name,
    '' as location_descriptor,
    '' as house_prefix_1,
    ifnull ( cast ( fromStreetNumber as varchar ) , '' ) as house_number_1,
    ifnull ( upper ( cast ( fromStreetNumberSuffix as varchar ) ) , '' ) as house_suffix_1,
    '' as house_prefix_2,
    ifnull ( cast ( toStreetNumber as varchar ) , '' ) as house_number_2,
    ifnull ( upper ( cast ( toStreetNumberSuffix as varchar ) ) , '' ) as house_suffix_2,
    ifnull ( upper ( streetNameOnly ) , '' ) as road_name,
    ifnull ( upper ( streetType ) , '' ) as road_type,
    case
        when upper ( streetName ) like '% NORTH' then 'N'
        when upper ( streetName ) like '% SOUTH' then 'S'
        when upper ( streetName ) like '% EAST' then 'E'
        when upper ( streetName ) like '% WEST' then 'W'
        else ''
    end as road_suffix,

...
set
    ...
    easting =  ifnull ( ( select round ( X ( geometry ) , 0 ) from lsc_rural_address ra where ra.prop_no = pc_council_property_address.propnum and ra.rural_no = pc_council_property_address.house_number_1 ) , '' ) ,
    northing = ifnull ( ( select round ( Y ( geometry ) , 0 ) from lsc_rural_address ra where ra.prop_no = pc_council_property_address.propnum and ra.rural_no = pc_council_property_address.house_number_1 ) , '' ) ,
